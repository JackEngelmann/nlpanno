...
def get_samples(
	...
	"""Get all samples."""
	all_samples = request_context.sample_repository.get_all()
...
def get_next_sample(
	...
	not_labeled = request_context.sample_repository.get_unlabeled()
	if len(not_labeled) == 0:
		return None

	sample_id = request_context.sampler(not_labeled)
	sample = request_context.sample_repository.get_by_id(sample_id)
...
def patch_sample(
	...
	request_context.sample_repository.update(updated_sample)
